...
func Start(context process.Context) {
	...
	log.WithFields("process-id", process.ID.Hex(), "PID", process.ProcessID,"process-name","worker").WriteInfo("PROCESS STARTED")
	...
	log.WithFields("api-key", user.APIKey,"process-name","worker").WriteInfo("ROOT USER LOGGED IN")
