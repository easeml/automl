...
func Start(context process.Context) {
	...
	process, err = modelContext.StartProcess(types.ProcScheduler)
	if err != nil {
		log.WriteFatal(fmt.Sprintf("fatal: %+v", err))
	}
	defer modelContext.SetProcessStatus(process.ID, types.ProcTerminated)
	log.WithFields("process-id", process.ID.Hex(), "PID", process.ProcessID,"process-name","scheduler").WriteInfo("PROCESS STARTED")
	...
	log.WithFields("api-key", user.APIKey,"process-name","scheduler").WriteInfo("ROOT USER LOGGED IN")
