...
func TestGetTasks(t *testing.T) {
	...
	assert.ElementsMatch([]types.Task{task1, task2}, result)
	...
	assert.Nil(err)
	assert.ElementsMatch([]types.Task{task1}, result)
	...
	assert.Nil(err)
	assert.ElementsMatch([]types.Task{task2}, result)
	assert.Equal(1, cm.TotalResultSize)
	assert.Equal(1, cm.ReturnedResultSize)
	assert.Equal("", cm.NextPageCursor)
	...
	assert.Nil(err)
	assert.Equal([]types.Task{task2, task1}, result)
	assert.Equal(2, cm.TotalResultSize)
	assert.Equal(2, cm.ReturnedResultSize)
	assert.Equal("", cm.NextPageCursor)
