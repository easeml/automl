...
var createJobCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		objectives, err := context.GetModules(types.ModuleObjective, "", types.ModuleActive, "", dataset.SchemaOut, "")
		if err != nil {
			fmt.Println("Error: " + err.Error())
			return
		}
		...
		models, err := context.GetModules(types.ModuleModel, "", types.ModuleActive, "", dataset.SchemaIn, dataset.SchemaOut)
		if err != nil {
			fmt.Println("Error: " + err.Error())
			return
		}
...
func init() {
	...
	createJobCmd.Flags().Uint64Var(&jobMaxTasks, "max-tasks", types.DefaultMaxTasks, "Maximum number of tasks to spawn from this job.")
