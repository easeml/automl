...
func (context Context) GetDatasetByID(id string) (result Dataset, err error) {
...
func (context Context) GetDatasets(
	...
	sortBy string,
	order string,
) (result []Dataset, cm CollectionMetadata, err error) {
	...
	if schInput != nil || schOutput != nil {
		var allResultsFiltered []Dataset
...
func (context Context) CreateDataset(dataset Dataset) (result Dataset, err error) {
	...
	if dataset.Source != DatasetUpload && dataset.Source != DatasetLocal && dataset.Source != DatasetDownload {
		err = errors.Wrapf(ErrBadInput,
			"value of source can be \"%s\", \"%s\" or \"%s\", but found \"%s\"",
			DatasetUpload, DatasetLocal, DatasetDownload, dataset.Source)
		return
	}
...
func (context Context) UpdateDataset(id string, updates map[string]interface{}) (result Dataset, err error) {
	...
	for k, v := range updates {
		...
		case "status":
			...
			if status != DatasetCreated &&
				status != DatasetTransferred &&
				status != DatasetUnpacked &&
				status != DatasetValidated &&
				status != DatasetArchived &&
				status != DatasetError {
				err = errors.Wrapf(ErrBadInput,
					"value of status can be \"%s\", \"%s\", \"%s\", \"%s\", \"%s\" or \"%s\", but found \"%s\"",
					DatasetCreated, DatasetTransferred, DatasetUnpacked, DatasetValidated, DatasetArchived, DatasetError, status)
				return
			}
...
func (context Context) ReleaseDatasetLockByProcess(processID bson.ObjectId) (numReleased int, err error) {
	...
	changeInfo, err = c.UpdateAll(
		bson.M{"process": processID, "status": bson.M{"$ne": DatasetError}},
