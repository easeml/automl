...
func TestUserAuthenticate(t *testing.T) {
	...
	err = c.Insert(User{ObjectID: bson.NewObjectId(), ID: "user1", PasswordHash: "hash1", Status: "active", APIKey: "apikey1"})
	assert.Nil(err)

	// Authenticate with API Key.
	user, err := context.UserAuthenticate(User{APIKey: "apikey1"})
	assert.Nil(err)
	assert.Equal("user1", user.ID)

	// If the API Key is wrong, we should get an ErrWrongAPIKey error.
	user, err = context.UserAuthenticate(User{APIKey: "apikey11"})
	assert.Equal(ErrWrongAPIKey, err)

	// Authenticate with user ID and password hash.
	user, err = context.UserAuthenticate(User{ID: "user1", PasswordHash: "hash1"})
	assert.Nil(err)
	assert.Equal("apikey1", user.APIKey)

	// If the user ID or password are wrong, we should get an ErrWrongCredentials error.
	user, err = context.UserAuthenticate(User{ID: "user11", PasswordHash: "hash1"})
	assert.Equal(ErrWrongCredentials, err)
	assert.Empty(user.ID)
	user, err = context.UserAuthenticate(User{ID: "user1", PasswordHash: "hash11"})
	assert.Equal(ErrWrongCredentials, err)
	assert.Empty(user.ID)
...
func TestUserGenerateAPIKey(t *testing.T) {
	...
	connection, err := database.Connect(MongoInstance, TestDBName)
	assert.Nil(err)
	var user = User{ObjectID: bson.NewObjectId(), ID: "user1", PasswordHash: "hash1", Status: "active"}
...
func TestGetUserByID(t *testing.T) {
	...
	connection, err := database.Connect(MongoInstance, TestDBName)
	assert.Nil(err)
	var user = User{ObjectID: bson.NewObjectId(), ID: "user1", PasswordHash: "hash1", Status: "active", APIKey: "apikey1"}
	var context = Context{Session: connection.Session, DBName: connection.DBName, User: User{ID: UserRoot}}
...
func TestGetUsers(t *testing.T) {
	...
	connection, err := database.Connect(MongoInstance, TestDBName)
	assert.Nil(err)
	var user1 = User{ObjectID: bson.NewObjectId(), ID: "user1", PasswordHash: "hash1", Status: "active", APIKey: "apikey1"}
	var user2 = User{ObjectID: bson.NewObjectId(), ID: "user2", PasswordHash: "hash2", Status: "archived", APIKey: "apikey2"}
	var context = Context{Session: connection.Session, DBName: connection.DBName, User: User{ID: UserRoot}}
	...
	assert.Nil(err)
	assert.ElementsMatch([]User{user1, user2}, result)
	...
	assert.Nil(err)
	assert.ElementsMatch([]User{user1}, result)
	...
	assert.Nil(err)
	assert.ElementsMatch([]User{user2}, result)
	assert.Equal(1, cm.TotalResultSize)
	assert.Equal(1, cm.ReturnedResultSize)
	assert.Equal("", cm.NextPageCursor)
	...
	assert.Nil(err)
	assert.Equal([]User{user2, user1}, result)
	assert.Equal(2, cm.TotalResultSize)
	assert.Equal(2, cm.ReturnedResultSize)
	assert.Equal("", cm.NextPageCursor)
	...
	assert.Nil(err)
	assert.Equal([]User{user1, user2}, result)
	assert.Equal(2, cm.TotalResultSize)
	assert.Equal(2, cm.ReturnedResultSize)
	assert.Equal("", cm.NextPageCursor)
...
func TestCreateUser(t *testing.T) {
	...
	newUser, err = context.CreateUser(User{ID: UserRoot})
	assert.Equal(ErrBadInput, errors.Cause(err))
