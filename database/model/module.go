...
func (context Context) GetModuleByID(id string) (result Module, err error) {
...
func (context Context) GetModules(
	...
	sortBy string,
	order string,
) (result []Module, cm CollectionMetadata, err error) {
...
func (context Context) CreateModule(module Module) (result Module, err error) {
	...
	if module.Source != ModuleUpload &&
		module.Source != ModuleLocal &&
		module.Source != ModuleRegistry &&
		module.Source != ModuleDownload {
		err = errors.Wrapf(ErrBadInput,
			"value of source can be \"%s\", \"%s\", \"%s\" or \"%s\", but found \"%s\"",
			ModuleUpload, ModuleLocal, ModuleDownload, ModuleRegistry, module.Source)
		return
	}
	if module.Type != ModuleModel &&
		module.Type != ModuleObjective &&
		module.Type != ModuleOptimizer {
		err = errors.Wrapf(ErrBadInput,
			"value of type can be \"%s\", \"%s\" or \"%s\", but found \"%s\"",
			ModuleModel, ModuleObjective, ModuleOptimizer, module.Type)
		return
	}
...
func (context Context) UpdateModule(id string, updates map[string]interface{}) (result Module, err error) {
	...
	for k, v := range updates {
		...
		case "status":
			...
			if status != ModuleCreated &&
				status != ModuleTransferred &&
				status != ModuleActive &&
				status != ModuleArchived &&
				status != ModuleError {
				err = errors.Wrapf(ErrBadInput,
					"value of status can be \"%s\", \"%s\", \"%s\", \"%s\" or \"%s\", but found \"%s\"",
					ModuleCreated, ModuleTransferred, ModuleActive, ModuleArchived, ModuleError, status)
				return
			}
...
func (context Context) ReleaseModuleLockByProcess(processID bson.ObjectId) (numReleased int, err error) {
	...
	changeInfo, err = c.UpdateAll(
		bson.M{"process": processID, "status": bson.M{"$ne": ModuleError}},
