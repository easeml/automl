...
func TestGetDatasets(t *testing.T) {
	...
	assert.Nil(err)
	assert.ElementsMatch([]Dataset{dataset1, dataset2}, result)
	...
	assert.Nil(err)
	assert.ElementsMatch([]Dataset{dataset2}, result)
	assert.Equal(1, cm.TotalResultSize)
	assert.Equal(1, cm.ReturnedResultSize)
	assert.Equal("", cm.NextPageCursor)
	...
	assert.Nil(err)
	assert.Equal([]Dataset{dataset2, dataset1}, result)
	assert.Equal(2, cm.TotalResultSize)
	assert.Equal(2, cm.ReturnedResultSize)
	assert.Equal("", cm.NextPageCursor)
...
func TestCreateDataset(t *testing.T) {
	...
	newDataset, err = context.CreateDataset(Dataset{ID: "root/dataset1"})
	assert.Equal(ErrBadInput, errors.Cause(err))
