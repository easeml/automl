...
func TestGetModules(t *testing.T) {
	...
	assert.Nil(err)
	assert.ElementsMatch([]Module{module1, module2}, result)
	...
	assert.Nil(err)
	assert.ElementsMatch([]Module{module2}, result)
	assert.Equal(1, cm.TotalResultSize)
	assert.Equal(1, cm.ReturnedResultSize)
	assert.Equal("", cm.NextPageCursor)
	...
	assert.Nil(err)
	assert.Equal([]Module{module2, module1}, result)
	assert.Equal(2, cm.TotalResultSize)
	assert.Equal(2, cm.ReturnedResultSize)
	assert.Equal("", cm.NextPageCursor)
...
func TestCreateModule(t *testing.T) {
	...
	newModule, err = context.CreateModule(Module{ID: "root/module1"})
	assert.Equal(ErrBadInput, errors.Cause(err))
