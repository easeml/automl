...
func TestGetProcesses(t *testing.T) {
	...
	assert.Nil(err)
	assert.ElementsMatch([]Process{process1, process2}, result)
	...
	assert.Nil(err)
	assert.ElementsMatch([]Process{process1}, result)
	...
	assert.Nil(err)
	assert.ElementsMatch([]Process{process2}, result)
	assert.Equal(1, cm.TotalResultSize)
	assert.Equal(1, cm.ReturnedResultSize)
	assert.Equal("", cm.NextPageCursor)
	...
	assert.Nil(err)
	assert.Equal([]Process{process2, process1}, result)
	assert.Equal(2, cm.TotalResultSize)
	assert.Equal(2, cm.ReturnedResultSize)
	assert.Equal("", cm.NextPageCursor)
	...
	assert.Nil(err)
	assert.Equal([]Process{process1, process2}, result)
	assert.Equal(2, cm.TotalResultSize)
	assert.Equal(2, cm.ReturnedResultSize)
	assert.Equal("", cm.NextPageCursor)
...
func TestCreateProcess(t *testing.T) {
	...
	newProcess, err = context.CreateProcess(Process{ID: process.ID})
	assert.Equal(ErrBadInput, errors.Cause(err))
...
func TestPatchProcess(t *testing.T) {
	...
	assert.Equal(ErrUnauthorized, errors.Cause(err))
