...
func (apiContext Context) DatasetsUploadHandler(basePath string) http.HandlerFunc {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if dataset.Source != model.DatasetUpload {
		...
		if dataset.Status != model.DatasetCreated {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if arch != nil {
			...
			if err != nil && os.IsNotExist(err) == false {
				...
				http.ServeContent(w, r, relativePath, dataset.CreationTime, bytes.NewReader(b.Bytes()))
				return
			}
		}
