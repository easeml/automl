...
func (context Context) ModuleValidateListener() {
	...
	for {
		module, err := context.ModelContext.LockModule(model.F{"status": model.ModuleTransferred}, context.ProcessID, "", "")
		if err == nil {
...
// config space and
func (context Context) ModuleValidateWorker(module model.Module) {
	...
	if module.Type == model.ModuleModel && (schemaIn == nil || schemaOut == nil) {
		err = errors.New("module of type model must have an input and output schema")
		err = errors.WithStack(err)
		context.moduleValidationError(err, module)
		return

	} else if module.Type == model.ModuleObjective && schemaIn == nil {
	...
	context.repeatUntilSuccess(func() error {
		return context.ModelContext.UpdateModuleStatus(module.ID, model.ModuleActive, "")
	})
	...
	if module.Type == model.ModuleModel {
...
func (context Context) moduleValidationError(err error, module model.Module) {
	context.Logger.WithFields(
		"module-id", module.ID,
		"source", module.Source,
		"source-address", module.SourceAddress,
