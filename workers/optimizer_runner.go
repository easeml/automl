...
func (context Context) OptimizerRunListener(optimizerID string) {
	...
	for {
		...
		numJobs, err := context.ModelContext.CountJobs(model.F{"status": model.JobRunning})
		if err != nil {
			panic(err)
		}
		...
		if numJobs > 0 {
			...
			idleCount, err := context.ModelContext.CountProcesses(model.F{"type": model.ProcWorker, "status": model.ProcIdle})
			if err != nil {
				panic(err)
			}
			workingCount, err := context.ModelContext.CountProcesses(model.F{"type": model.ProcWorker, "status": model.ProcWorking})
			if err != nil {
				panic(err)
			}
			numTasks, err := context.ModelContext.CountTasks(model.F{"status": model.TaskScheduled})
			if err != nil {
				panic(err)
			}
...
func (context Context) OptimizerRunWorker(optimizerID string, numProcesses, numTasks int) {
	...
	// Get optimizer image.
	imageFilePath := context.getModuleImagePath(optimizerID, model.ModuleOptimizer)
	...
	jobs, _, err := context.ModelContext.GetJobs(model.F{"status": model.JobRunning}, 0, "", "", "")
	if err != nil {
		panic(err)
	}
