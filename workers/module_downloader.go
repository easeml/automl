...
func (context Context) ModuleDownloadListener() {
	...
	for {
		module, err := context.ModelContext.LockModule(model.F{"source": model.ModuleDownload, "status": model.ModuleCreated}, context.ProcessID, "", "")
		if err == nil {
		...
		module, err = context.ModelContext.LockModule(model.F{"source": model.ModuleLocal, "status": model.ModuleCreated}, context.ProcessID, "", "")
		if err == nil {
		...
		module, err = context.ModelContext.LockModule(model.F{"source": model.ModuleRegistry, "status": model.ModuleCreated}, context.ProcessID, "", "")
		if err == nil {
...
func (context Context) ModuleDownloadWorker(module model.Module) {
	...
	if err != nil {
		...
		context.repeatUntilSuccess(func() error {
			return context.ModelContext.UpdateModuleStatus(module.ID, model.ModuleError, err.Error())
		})
	...
	context.repeatUntilSuccess(func() error {
		return context.ModelContext.UpdateModuleStatus(module.ID, model.ModuleTransferred, "")
	})
	context.repeatUntilSuccess(func() error {
...
func (context Context) ModuleLocalCopyWorker(module model.Module) {
...
func (context Context) ModuleRegistryPullWorker(module model.Module) {
	...
	resp, err := cli.ImagePull(ctx.Background(), module.SourceAddress, types.ImagePullOptions{})
	defer resp.Close()
	if err != nil {
		panic(err)
	}
