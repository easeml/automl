...
func (context Context) DatasetUnpackListener() {
	...
	for {
		dataset, err := context.ModelContext.LockDataset(model.F{"status": model.DatasetTransferred}, context.ProcessID, "", "")
		if err == nil {
...
func (context Context) DatasetUnpackWorker(dataset model.Dataset) {
	...
	if dataset.Source == model.DatasetDownload {
	...
	} else if dataset.Source == model.DatasetUpload {
	...
	} else if dataset.Source == model.DatasetLocal {
	...
	for i := range sourceFilePaths {
		...
		if arch == nil {
			...
			_, err := context.ModelContext.UpdateDataset(dataset.ID, model.F{"status": model.DatasetError, "status-message": err.Error()})
			if err != nil {
				panic(err)
			}
	...
	context.repeatUntilSuccess(func() error {
		return context.ModelContext.UpdateDatasetStatus(dataset.ID, model.DatasetUnpacked, "")
	})
	context.repeatUntilSuccess(func() error {
