...
func (context Context) DatasetDownloadListener() {
	...
	for {
		dataset, err := context.ModelContext.LockDataset(model.F{"source": model.DatasetDownload, "status": model.DatasetCreated}, context.ProcessID, "", "")
		if err == nil {
		...
		dataset, err = context.ModelContext.LockDataset(model.F{"source": model.DatasetLocal, "status": model.DatasetCreated}, context.ProcessID, "", "")
		if err == nil {
...
func (context Context) DatasetDownloadWorker(dataset model.Dataset) {
	...
	context.repeatUntilSuccess(func() error {
		return context.ModelContext.UpdateDatasetStatus(dataset.ID, model.DatasetTransferred, "")
	})
	context.repeatUntilSuccess(func() error {
...
func (context Context) DatasetLocalCopyWorker(dataset model.Dataset) {
