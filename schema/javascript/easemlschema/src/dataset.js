    ...
    for (let node in allInstances) {
      ...
      for (let i = 0; i < instances.length; i++) {
        for (let target in schema.nodes[node].links) {
          ...
          let lBound = link.dim[0]
          let uBound = link.dim[1]
          if (uBound === 'inf' || uBound > numNodeInstances) {
            uBound = numNodeInstances
          }
          assert(lBound <= uBound)

          let count = Math.floor(Math.random() * (uBound - lBound + 1)) - lBound
...
function dumpTensor (self, root, relPath, name, opener) {
  ...
  if (self.subtype === 'default') {
    ...
    throw new DatasetException("Unknown tensor subtype '" + self.subtype + "'.", filePath)
  }
}
...
Links.prototype.adjacencyMap = function (nodes) {
  ...
  for (let [node, adj] in adjacency.entries()) {
