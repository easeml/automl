...
def prediction (Queue, GridSize, DB, DE, T, n_parallel, chunk_size, ParamOBS, ParamDATA, ADDRESSprep, ListParam, TopoListParam, n_gpus, num_epochs, num_batch, with_chunk_randomization, withTopo, modelName):
    ...
    model = cosmoe_controlmodel.cosmoe_model(n_gpus, num_epochs, num_batch, with_chunk_randomization, modelName)
    ...
    while (done):
        while not Queue.empty():
            ...
            X, y_obs, y_pred = cosmoe_datapreparation_simplemodel.convertData(freshScaler = 0, trainingRun = 0, data = data, modelName = modelName) # convert data
