...
# Description: Method loading time invariant data
# Input: path to file
# Output: time invariant data from station, time invariante data from grid
def load_time_invariant_data(processId, Name):
    time_start = time()
    time_invariant_features = None
    with open(Name,"rb") as cur_file:
        time_invariant_features = pkl.load(cur_file)        
    station_position_data = time_invariant_features['station_position'].data # dimensions: station, feature)
    station_grid_data = time_invariant_features['grid_data'].data # dimensions: station, idx_lat, idx_lon, feature
    time_end = time()
    # print(('[Process %s] ' + 'time invariant features' + ' opened in %s') % (processId, round(time_end-time_start,2)))
    return station_position_data, station_grid_data
...
def getFilesToProcess(ADDRESSdata, DESTINATION, ListParam, Preprocessing, DateBegin, DateEnd):
    ...
    # list of all preprocessed chunks in the following form: (start chunk, end chunk, filename)
    all_chunks = [(D[:10], D[11:21], D) for D in os.listdir(ADDRESSdata)]

    return sorted(all_chunks, key=lambda tup: tup[0])
