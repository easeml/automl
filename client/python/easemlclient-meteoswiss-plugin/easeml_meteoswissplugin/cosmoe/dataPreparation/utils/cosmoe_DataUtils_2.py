...
def getFilesToProcess(ADDRESSdata, DESTINATION, ListParam, Preprocessing, DateBegin, DateEnd):
    ...
    for f in (sorted(all_cosmoe_files) + sorted(all_cosmoe_refcst_files) + sorted(all_cosmoe_pp_veri_files)):
        # add param_dict to initialization date if not seen yet
        # print(f)
        if f[0] not in folders.keys():
            param_dict = {}
            for i in ListParam:
                param_dict[i]= None
            folders[f[0]] = param_dict

        cur_param = getCosmoeParameter(ListParam,f[1])
        if cur_param == 0:
            continue
        if folders[f[0]][cur_param] is None:
            folders[f[0]][cur_param] = f[1]
