...
func (context Context) GetDatasets(status, source, schemaIn, schemaOut string) (result []model.Dataset, err error) {
...
func (context Context) GetDatasetByID(id string) (result *model.Dataset, err error) {
...
func DatasetSourceAddressRequired(source string) bool {
	switch source {
	case model.DatasetUpload:
		return false
	case model.DatasetLocal:
		return true
	case model.DatasetDownload:
		return true
	default:
...
func (context Context) UploadDataset(id, sourcePath string) error {
	...
	err = context.UpdateDataset(id, map[string]interface{}{"status": model.DatasetTransferred})
	if err != nil {
